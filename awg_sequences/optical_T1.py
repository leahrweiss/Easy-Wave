...
def genseq(init_t =2e-3, read_t = 10e-6, tend=500e-6, reset_t = 500e-6, read_delay=10e-6, npoints=100, rate = 1e8):
	...
	rtimes = np.linspace(0,tend,npoints)
	...
	for i,rtime in enumerate(rtimes):
		...
		line = Rect(t=init_t,ch=chs['aom'])+Zero(t=rtime,ch=chs['aom'])+Rect(t=init_t,ch=chs['aom'])+Zero(t=wait_t,ch=chs['aom'])
		...
		writer.add_line(line,'t1_'+str(i))



	writer.generate_and_upload('192.168.0.251', 'temp/optical_T1_500usreset.awg',rate,limits=limits_awg)
	return writer
