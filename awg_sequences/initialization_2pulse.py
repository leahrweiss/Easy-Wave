...
def genseq(wait_t = 10e-6, init_t = 500e-6, reset_t = 100e-3, rate = 1e7):
	...
	seq_len = init_t*2+wait_t+reset_t
	# make first line an AOM on/READ on line
	line1 = Rect(t=seq_len,ch = chs['aom'])
	line1 &= Rect(t=seq_len,ch = chs['readout'])
	writer.add_line(line1,'AOM ON/READ ON')


	init_line = Rect(t=init_t, ch=chs['aom'])+Zero(t=wait_t, ch=chs['aom'])+Rect(t=init_t, ch=chs['aom'])+Zero(t=reset_t, ch=chs['aom'])
	init_line &=Rect(t=init_t, ch=chs['swabian'])+Zero(t=(reset_t+init_t+wait_t), ch=chs['swabian'])
	
	# rtimes = np.array([10e-6,450e-6])
	# for i,rtime in enumerate(rtimes):
	# 	line = init_line
	# 	zero_len = seq_len-rtime-read_t
	# 	read_line = Zero(t = rtime, ch = chs['readout']) + Rect(t= read_t, ch = chs['readout']) + Zero(t = zero_len, ch = chs['readout'])
	# 	line &= read_line
	writer.add_line(init_line,'2pulse')



	writer.generate_and_upload('192.168.0.251', 'temp/initialization_swabian.awg',rate,limits=limits_awg)
	return writer
