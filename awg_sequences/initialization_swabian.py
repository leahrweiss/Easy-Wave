...
def genseq(buffer_t = 0, init_t =500e-6, read_t = 10e-6, reset_t = 100e-3, tau = 10e-6, rate = 1e7):
	...
	init_line &=Zero(t=buffer_t, ch=chs['swabian'])+Rect(t=init_t, ch=chs['swabian'])+Zero(t=buffer_t, ch=chs['swabian'])+Zero(t=reset_t, ch=chs['swabian'])
	...
	for i,rtime in enumerate(rtimes):
		...
		line &= read_line
		writer.add_line(line,'init_'+str(i))
